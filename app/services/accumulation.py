...
def _log(db: Session, event: str, symbol: str, message: str) -> None:
    db.add(ActivityLog(event_type=f"LIVE_{event}", symbol=symbol or "-", message=message))
...
def _live_buy(db: Session, plan: AccumulationPlan, usdt: float, reason: str) -> bool:
    ...
    if get_usdt_free() + 1e-9 < usdt:
        _log(db, "ACC_SKIP", plan.symbol, f"Plan={plan.name} | reason=insufficient_exchange_cash | need={usdt:.2f}")
        return False
    try:
        ...
        if float(order.get("quote_qty", 0.0)) <= 0.0:
            if settings.live_entry_limit_fallback_market:
                ...
                _log(db, "ACC_SKIP", plan.symbol, f"Plan={plan.name} | reason=limit_no_fill")
                return False
    except Exception:
        if settings.live_entry_limit_fallback_market:
            order = place_market_buy_quote(plan.symbol, usdt)
        else:
            _log(db, "ACC_SKIP", plan.symbol, f"Plan={plan.name} | reason=limit_failed_no_fallback")
            return False
...
def _live_sell(db: Session, plan: AccumulationPlan, qty: float, reason: str) -> bool:
    ...
    try:
        ...
        _log(db, "ACC_SELL_FAIL", plan.symbol, f"Plan={plan.name} | reason={reason} | error={e}")
        return False
...
def create_plan(
    ...
    *,
    ...
    db.add(plan)
    db.flush()
    _log(db, "ACC_PLAN_CREATE", plan.symbol, f"Plan={plan.name} | capital={plan.total_capital_usdt:.2f} | entry={plan.initial_entry_usdt:.2f}")
    return plan
...
def _run_plan_cycle(db: Session, plan: AccumulationPlan, price: float) -> bool:
    ...
    if float(plan.coin_qty or 0.0) <= 0:
        ...
        if init_amt >= float(plan.min_order_usdt or 5.0):
            ok = _live_buy(db, plan, init_amt, "initial_entry")
    ...
    if tp_pct > 0 and sell_pct > 0:
        ...
        if price >= sell_trigger:
            ...
            if sell_qty > 0 and sell_quote + 1e-12 >= float(plan.min_order_usdt or 5.0):
                ok = _live_sell(db, plan, sell_qty, "partial_take_profit")
                changed = changed or ok
                if ok:
                    return True
            elif sell_qty > 0:
    ...
    if price <= buy_trigger:
        ...
        if dca_amt >= float(plan.min_order_usdt or 5.0):
            ok = _live_buy(db, plan, dca_amt, "dca_buy")
            changed = changed or ok
    return changed


def run_accumulation_cycle(db: Session) -> None:
    plans = db.query(AccumulationPlan).filter(AccumulationPlan.mode == "live").all()
    if not plans:
        return
...
def toggle_plan_status(db: Session, plan: AccumulationPlan) -> str:
    ...
    plan.last_action_at = datetime.utcnow()
    _log(db, "ACC_PLAN_STATUS", plan.symbol, f"Plan={plan.name} | status={plan.status}")
