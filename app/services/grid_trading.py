...
def _log(db: Session, event: str, symbol: str, msg: str) -> None:
    db.add(ActivityLog(event_type=f"LIVE_GRID_{event}", symbol=symbol or "-", message=msg))
...
def _live_buy(db: Session, bot: GridBot, grid_index: int, usdt: float, reason: str) -> bool:
    ...
    if get_usdt_free() + 1e-9 < usdt:
        _log(db,"SKIP", bot.symbol, f"Bot={bot.name} | reason=insufficient_cash | need={usdt:.2f}")
        return False
    try:
        order = place_market_buy_quote(bot.symbol, usdt)
    except Exception as e:
        _log(db,"BUY_FAIL", bot.symbol, f"Bot={bot.name} | grid={grid_index} | error={e}")
        return False
    ...
    _log(db,"BUY", bot.symbol, f"Bot={bot.name} | grid={grid_index} | price={price:.6f} | spent={spent:.2f} | qty={qty:.8f}")
    return True
...
def _live_sell(db: Session, bot: GridBot, grid_index: int, qty: float, reason: str) -> bool:
    ...
    try:
        ...
        _log(db,"SELL_FAIL", bot.symbol, f"Bot={bot.name} | grid={grid_index} | error={e}")
        return False
    ...
    _log(db,"SELL", bot.symbol, f"Bot={bot.name} | grid={grid_index} | qty={exec_qty:.8f} | quote={quote:.2f} | pnl={pnl:+.2f}")
    return True


def _do_buy(db: Session, bot: GridBot, grid_index: int, reason: str) -> bool:
    usdt_per_grid = float(bot.total_investment_usdt) / max(1, int(bot.grid_count))
...
def _run_bot_cycle(db: Session, bot: GridBot, price: float) -> bool:
    ...
    if bot.trigger_price and bot.status == "waiting":
        ...
        _log(db,"TRIGGERED", bot.symbol, f"Bot={bot.name} | trigger_price={tp:.6f} | current={price:.6f}")
    ...
    if bot.last_grid_index is None:
        ...
        _log(db,"INIT", bot.symbol, f"Bot={bot.name} | price={price:.6f} | anchor_grid={bot.last_grid_index}")
        return True

    # Stop Loss check
    if bot.stop_loss_price and price <= float(bot.stop_loss_price):
        _log(db,"STOP_LOSS", bot.symbol, f"Bot={bot.name} | price={price:.6f} | sl={bot.stop_loss_price:.6f}")
        _close_bot(db, bot, "stop_loss")
        return True

    # Take Profit check
    if bot.take_profit_price and price >= float(bot.take_profit_price):
        _log(db,"TAKE_PROFIT", bot.symbol, f"Bot={bot.name} | price={price:.6f} | tp={bot.take_profit_price:.6f}")
        _close_bot(db, bot, "take_profit")
        return True

    # Price out of range
    if price < float(bot.lower_limit) or price > float(bot.upper_limit):
        _log(db,"OUT_OF_RANGE", bot.symbol, f"Bot={bot.name} | price={price:.6f} | range=[{bot.lower_limit},{bot.upper_limit}]")
        bot.last_grid_index = _grid_index_for_price(levels, price)
        return False
    ...
    if cur_idx > last_idx:
        ...
        for i in range(last_idx, cur_idx, -1):
            if fills >= _MAX_FILLS_PER_CYCLE:
                break
            ok = _do_buy(db, bot, i - 1, "grid_buy")
            if ok:
                fills += 1
                changed = True
...
def _close_bot(db: Session, bot: GridBot, reason: str) -> None:
...
def create_bot(
    ...
    *,
    ...
    db.add(bot)
    db.flush()
    _log(db,"CREATE", bot.symbol, f"Bot={bot.name} | capital={bot.total_investment_usdt:.2f} | grids={bot.grid_count} | range=[{bot.lower_limit},{bot.upper_limit}]")
    return bot


def run_grid_cycle(db: Session) -> None:
    bots = (
        db.query(GridBot)
        .filter(GridBot.mode == "live")
...
def toggle_bot_status(db: Session, bot: GridBot) -> str:
    ...
    bot.last_action_at = datetime.utcnow()
    _log(db,"STATUS", bot.symbol, f"Bot={bot.name} | status={bot.status}")
