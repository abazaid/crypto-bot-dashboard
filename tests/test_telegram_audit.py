...
def test_simulate_all_targets_done():
    ...
    kl = [
        _k(0, 0.150, 0.152, 0.148, 0.151),  # entry candle @ open 0.150; low touches the zone bottom -> both legs, avg 0.149
...
def test_simulate_split_entry_scales_pnl_by_invested_share():
...
def test_simulate_stop_before_target_is_a_loss():
    ...
    res = simulate(sig, kl, 0, 12, entry_split=0.0)
...
def test_simulate_missed_and_open():
    ...
    res = simulate(sig, kl2, 0, 12, entry_split=0.0)
...
def test_simulate_lock_zero_means_previous_target():
    ...
    tight = simulate(sig, kl, 0, 12, target_lock=0.5, giveback=1.0, entry_split=0.0)
